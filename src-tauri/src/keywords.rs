...
fn extract_english_words(text: &str) -> Vec<String> {
    // 匹配连续的英文字母（包括大写开头的词），静态缓存避免每次重新编译正则
    static EN_WORD_RE: OnceLock<Regex> = OnceLock::new();
    let re = EN_WORD_RE.get_or_init(|| Regex::new(r"[A-Za-z]{2,}").unwrap());
