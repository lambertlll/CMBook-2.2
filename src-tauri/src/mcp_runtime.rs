...
pub fn install_recipe_for(kind: &RuntimeKind, platform: &str) -> Option<InstallRecipe> {
    match (kind, platform) {
        (RuntimeKind::Uvx, "macos") => Some(InstallRecipe {
            ...
            post_install_hint: Some("If uv is still unavailable after installation, restart CMBook or open a new terminal session and re-check."),
            scope: "current_user",
            manual_only: false,
        }),
        ...
        (RuntimeKind::Bunx, "macos") => Some(InstallRecipe {
            ...
            post_install_hint: Some("Bun installs into ~/.bun/bin. If bun is still unavailable after installation, add that directory to PATH, then restart CMBook or open a new terminal session and re-check."),
            scope: "current_user",
            manual_only: false,
        }),
        (RuntimeKind::Bunx, "linux") => Some(InstallRecipe {
            ...
            title: "Install Bun",
            command_preview: "curl -fsSL https://bun.com/install | bash",
            post_install_hint: Some("Bun installs into ~/.bun/bin and requires unzip on Linux. If bun is still unavailable after installation, add that directory to PATH, then restart CMBook or open a new terminal session and re-check."),
            scope: "current_user",
            manual_only: false,
        }),
        (RuntimeKind::Bunx, "windows") => Some(InstallRecipe {
            ...
            post_install_hint: Some("If bun is still unavailable after installation, restart CMBook or open a new terminal session and re-check your PATH."),
            scope: "current_user",
            manual_only: false,
        }),
        (RuntimeKind::Npx, "macos") | (RuntimeKind::Npx, "linux") => Some(InstallRecipe {
            ...
            post_install_hint: Some("Volta updates shell configuration for future sessions. If npx is still unavailable after installation, restart CMBook or open a new terminal session and re-check."),
            scope: "current_user",
            manual_only: false,
        }),
        (RuntimeKind::Npx, "windows") => Some(InstallRecipe {
            ...
            post_install_hint: Some("Windows may not expose Volta in the current session immediately. If npx is still unavailable after installation, restart CMBook or open a new terminal session and re-check."),
            scope: "current_user",
            manual_only: false,
        }),
        ...
        | (RuntimeKind::Python3, "linux") => Some(InstallRecipe {
            ...
            command_preview: "Install Python 3 in your user environment, then re-check in CMBook.",
            post_install_hint: Some("Use the official Python installer for your platform, then restart CMBook or open a new terminal session before re-checking."),
            scope: "current_user",
            manual_only: true,
        }),
        _ => None,
...
pub async fn install_mcp_runtime(
    ...
    // 先记录结果，再统一清理 active_installs，最后才返回错误，
    // 避免失败路径在清理前 return 导致残留
    ...
    let status =
        wait_result.map_err(|error| format!("Failed to wait for install recipe: {error}"))?;
    stdout_result.map_err(|error| format!("Failed to collect stdout task: {error}"))??;
    stderr_result.map_err(|error| format!("Failed to collect stderr task: {error}"))??;
