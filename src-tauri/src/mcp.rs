...
/// MCP 读写超时时间，防止服务不响应时 worker 永久阻塞
const MCP_IO_TIMEOUT: Duration = Duration::from_secs(60);
...
pub async fn start_mcp_stdio_server(
    ...
    // 检查是否已经启动，如果已启动则先停止
    {
        let mut processes = manager
            .processes
            .lock()
            .map_err(|e| format!("MCP process manager lock poisoned: {}", e))?;
    ...
    let mut cmd = if command == "npx" || command.ends_with("/npx") || command.ends_with("\\npx") {
        ...
        if let Some(npx) = npx_path {
            ...
            // 找不到 npx 时直接报错，避免把前端配置的 args 拼接进 shell 命令（注入风险）
            return Err(
                "找不到 npx 可执行文件。请先安装 Node.js 并将 npx 加入 PATH，或在 MCP 配置中直接填写 npx 的完整路径。"
                    .to_string(),
            );
        }
    } else {
...
pub async fn stop_mcp_server(
    ...
    if let Some(child) = child {
        let mut child = child
            .lock()
            .map_err(|e| format!("MCP process lock poisoned: {}", e))?;
        child
            .kill()
...
pub async fn send_mcp_message(
    ...
    // 只短暂持锁取出 stdin/stdout 句柄，不要跨 I/O 持锁，
    // 否则 MCP 服务不响应时 stop_mcp_server 会因同一把锁死锁
    let (stdin, stdout) = {
        let mut child = child
            .lock()
            .map_err(|e| format!("MCP process lock poisoned: {}", e))?;
    ...
    // 同步阻塞读写放进 spawn_blocking，并加超时兜底
    let io_result = tokio::time::timeout(
        MCP_IO_TIMEOUT,
        tauri::async_runtime::spawn_blocking(
            move || -> Result<(String, ChildStdin, ChildStdout), String> {
                let mut stdin = stdin;
    ...
    match io_result {
        Ok(join_result) => {
            let (response, stdin, stdout) = join_result
                .map_err(|e| format!("MCP I/O task failed to join: {}", e))??;
            // 读写完成，把句柄放回子进程
            {
                let mut child = child
                    .lock()
