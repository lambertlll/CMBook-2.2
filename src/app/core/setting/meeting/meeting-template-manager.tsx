...
/**
 * 会议纪要自定义模板管理：列表 + 新建/编辑 Dialog + 删除确认。
 * 模板存 setting store（store.json），生成纪要时在模板下拉中与内置模板一起选择。
 */
export function MeetingTemplateManager() {
  const t = useTranslations('settings.meeting.customTemplates')
  const customMeetingTemplates = useSettingStore((s) => s.customMeetingTemplates)
  const addCustomMeetingTemplate = useSettingStore((s) => s.addCustomMeetingTemplate)
  const updateCustomMeetingTemplate = useSettingStore((s) => s.updateCustomMeetingTemplate)
  const removeCustomMeetingTemplate = useSettingStore((s) => s.removeCustomMeetingTemplate)

  // 新建/编辑 Dialog 状态：editing 为 null 表示新建
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      await updateCustomMeetingTemplate(editing.id, { name: trimmedName, prompt: trimmedPrompt })
    } else {
      await addCustomMeetingTemplate({ name: trimmedName, prompt: trimmedPrompt })
    }
    setDialogOpen(false)
  }
  ...
  const handleDelete = async () => {
    ...
    await removeCustomMeetingTemplate(deleteTarget.id)
  ...
  return (
    <>
      ...
      {customMeetingTemplates.map((tpl) => (
