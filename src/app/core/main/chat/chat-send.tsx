...
export const ChatSend = forwardRef<{ sendChat: (content?: string) => void }, ChatSendProps>(({ inputValue, onSent, linkedResource, attachedImages = [], quoteData = null, dockStyle = false }, ref) => {
  ...
  // Agent 模式处理（text 为本次发送内容，可能与输入框状态无关——程序化发送直接携带）
  async function handleAgentMode(imageUrls: string[], text: string) {
    // 先创建一个占位的 AI 消息
...
`
      ...
      await agentHandler.execute(text, messages, imageUrls)
    } catch (error) {
  ...
  // 对话（Agent 模式）。overrideContent：程序化发送时直接携带的内容，
  // 不依赖输入框状态提交（替代原 100ms setTimeout 等待渲染的方案，消除竞态）
  async function handleSubmit(overrideContent?: string) {
    const text = typeof overrideContent === 'string' ? overrideContent : inputValue
    if (!text.trim()) return
    ...
    try {
      ...
      await handleAgentMode(imageUrls, text)
    } finally {
