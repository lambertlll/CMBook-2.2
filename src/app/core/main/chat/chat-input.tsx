...
import { ChatToolsDrawer } from "./chat-tools-drawer"
...
export const ChatInput = React.memo(function ChatInput() {
  ...
  const chatSendRef = useRef<{ sendChat: (content?: string) => void } | null>(null)
  ...
  useEffect(() => {
    ...
    // 程序化发送（如访前材料生成）：填入内容并自动触发发送
    emitter.on('send-chat-message', (event: unknown) => {
      const message = typeof event === 'string' ? event : (event as { content?: string })?.content
