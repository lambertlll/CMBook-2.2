...
export function TipTapEditor({
  ...
  // 居中内容设置（桌面端默认开启，读取 store.json 后校准；移动端强制关闭）
  const [centeredContent, setCenteredContent] = useState(true)
  ...
  useEffect(() => {
    async function loadCenteredContent() {
      ...
      // 未设置过时默认开启（居中排版）；已存值的老用户保持原设置
      const centered = (await store.get<boolean>('centeredContent')) ?? true
