...
function ItemIcon({ isLocked=false, isPin=false }) {
  if (isLocked) {
    return <Lock className="scale-75 text-muted-foreground" />
  } else {
    if (isPin) {
      return <Pin className="scale-75 text-muted-foreground" />
    } else {
      return <TagIcon className="scale-75 text-muted-foreground" />
    }
  }
}
