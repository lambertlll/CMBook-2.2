...
function getImageStatusClasses(status: string | null) {
  switch (status) {
  case 'pending':
    return "border-warning/70 bg-warning/10 text-warning dark:border-warning/40 dark:bg-warning/15 dark:text-warning"
  case 'failed':
    return "border-danger/70 bg-danger/10 text-danger dark:border-danger/40 dark:bg-danger/15 dark:text-danger"
  case 'noText':
    return "border-border/70 bg-muted text-foreground dark:border-border/40 dark:bg-muted/15 dark:text-muted-foreground"
  case 'savedOnly':
    return "border-primary/70 bg-accent text-primary dark:border-primary/40 dark:bg-primary/15 dark:text-primary"
  default:
    return "border-border/70 bg-muted text-foreground dark:border-border/40 dark:bg-muted/15 dark:text-muted-foreground"
  }
}
          ...
          {!isImageCard && mark.type === 'todo' && itemContent.todo ? (
            <div className="flex w-full min-w-0 max-w-full items-center gap-2 overflow-hidden text-xs text-muted-foreground">
              <div className="flex min-w-0 items-center gap-2 overflow-hidden">
                {itemContent.todo.completed ? <CheckSquare className="size-3.5 text-success" /> : <Square className="size-3.5 text-muted-foreground" />}
