...
export function useAiCompletion(options: UseAiCompletionOptions = {}) {
  ...
  const generateCompletion = useCallback(async (fullContent: string, cursorPosition: number) => {
    ...
    setIsLoading(true)
    // 保存本请求的 controller，finally 清理时比对，避免把新请求的 controller 置空
    const controller = new AbortController()
    abortControllerRef.current = controller

    try {
      const result = await fetchCompletion(context, controller.signal)
      ...
      // 只有当前 ref 仍指向本请求的 controller 时才清理，
      // 否则说明已有更新的请求接管，不能清掉它的状态
