...
function useUsername() {
  const primaryBackupMethod = useSettingStore(s => s.primaryBackupMethod)
  const userInfo = useSyncStore(s => s.userInfo)
  const giteeUserInfo = useSyncStore(s => s.giteeUserInfo)
  const gitlabUserInfo = useSyncStore(s => s.gitlabUserInfo)
  const giteaUserInfo = useSyncStore(s => s.giteaUserInfo)
