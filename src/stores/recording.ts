...
const useRecordingStore = create<RecordingState>((set, get) => ({
    ...
    try {
      ...
      if (error instanceof DOMException) {
        if (error.name === 'NotAllowedError') {
          throw new Error('麦克风权限被拒绝，请在系统设置中允许招本访问麦克风')
  ...
  resumeRecording: () => {
    ...
    if (mediaRecorder && mediaRecorder.state === 'paused') {
      ...
      // 恢复时重建计时器（pauseRecording 已清除，逻辑与 startRecording 一致）
      const timerId = setInterval(() => {
        const state = get()
        if (state.isRecording && !state.isPaused) {
          set({ recordingDuration: state.recordingDuration + 1 })
  ...
  resetState: () => {
    const { timerId, mediaRecorder } = get()
    // 重置时清除计时器
    if (timerId) {
      clearInterval(timerId)
    }
    // 统一兜底释放麦克风轨道（重复 stop 是安全的）
    mediaRecorder?.stream.getTracks().forEach(track => track.stop())
    set({
      isRecording: false,
      isPaused: false,
