...
import { isBuiltinSkill } from '@/lib/skills/builtin'
...
interface SkillsState {
  ...
  builtinSkillIds: string[]       // 内置 Skill ID 列表（存在 .builtin 标记文件）
...
export const useSkillsStore = create<SkillsState>((set, get) => ({
  ...
  refreshSkills: async () => {
    ...
    // 识别内置 Skills（全局 skill 目录中存在 .builtin 标记文件）
    const builtinChecks = await Promise.all(
      globalSkills.map(async (skill) => {
        ...
        const builtin = await isBuiltinSkill(
          fileInfo.directory,
          skill.metadata.scope
        )
        return builtin ? skill.metadata.id : null
      })
    )
    const builtinSkillIds = builtinChecks.filter(
