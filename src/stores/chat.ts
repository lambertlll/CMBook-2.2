  ...
  insert: async (chat) => {
    ...
    if (res.lastInsertId) {
      ...
      if (conversationId) {
        ...
        // 仅自动命名默认标题（"新对话"）的会话：已被显式命名的会话
        // （如拜访生成任务的"客户名·任务类型 MM-DD"、用户手动改名）不被首条消息覆盖
        if (currentConv && currentConv.messageCount === 1 && chat.role === 'user' && chat.content && currentConv.title === '新对话') {
          // 直接使用用户输入的前30个字符作为标题
