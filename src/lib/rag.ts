...
/**
 * 可索引文件类型判断（2.4 B2-3）：知识库向量索引支持 .md 与 .txt。
 * txt 按纯文本读取切块，与 md 同一管线；替换原先散落的 5 处 `.endsWith('.md')` 硬编码。
 */
export function isIndexableFile(fileName: string): boolean {
 ...
 * 失败安全：先完成全部切块嵌入并暂存，任一块失败都保留旧索引并返回 false；
 * 全部嵌入成功后才删旧插新，插入中途失败则用备份回滚旧索引。
 */
export async function processMarkdownFile(
  ...
  try {
    ...
    // 嵌入前先备份旧向量，供写库失败时回滚
    const backupDocs = await getVectorDocumentsByFilename(vectorDocumentKey);
    const legacyBackupDocs = legacyFilename !== vectorDocumentKey
      ? await getVectorDocumentsByFilename(legacyFilename)
      : [];

    // 先完成全部切块嵌入并暂存结果：任一块失败则保留旧索引，直接返回失败
    const embeddings: number[][] = [];
    for (let i = 0; i < chunks.length; i++) {
      const embedding = await fetchEmbedding(chunks[i]);
      if (!embedding) {
        console.error(`无法计算文件 ${vectorDocumentKey} 第 ${i+1} 块的向量，保留旧索引`);
        return false;
      }
    ...
    try {
      ...
      console.error(`写入文件 ${vectorDocumentKey} 的向量失败，回滚旧索引:`, error);
      await deleteVectorDocumentsByFilename(vectorDocumentKey);
      if (legacyFilename !== vectorDocumentKey) {
        await deleteVectorDocumentsByFilename(legacyFilename);
      }
      for (const doc of [...backupDocs, ...legacyBackupDocs]) {
        await upsertVectorDocument({
...
async function getWorkspaceFiles(): Promise<DirTree[]> {
  ...
  async function processDirectory(dirPath: string, useCustomPath: boolean): Promise<DirTree[]> {
    ...
    for (const entry of entries) {
      ...
      if (!entry.isDirectory && !isIndexableFile(entry.name)) continue;
  ...
  try {
    ...
    async function collectFiles(tree: DirTree[]): Promise<void> {
      for (const item of tree) {
        if (item.isFile && isIndexableFile(item.name)) {
...
export async function getContextForQueryInFolder(
  ...
  try {
    ...
    // 向量库的 filename 为工作区相对路径（getVectorDocumentKey），需同时按相对路径匹配，
    // 否则文件夹范围内的向量检索结果会被全部过滤掉（basename 与相对路径不匹配）
    const folderRelativePaths = new Set(items.map(item => item.id || ''));
    ...
    try {
      ...
      for (const keyword of sortedKeywords) {
        ...
        if (queryEmbedding) {
          let similarDocs = await getSimilarDocuments(queryEmbedding, resultCount * 2, similarityThreshold);
          // 过滤：只保留文件夹内的文件（向量库存相对路径，旧数据可能存 basename，两者都匹配）
          similarDocs = similarDocs.filter(doc =>
            folderRelativePaths.has(doc.filename) || folderFilenames.has(doc.filename)
          );
