  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        file_type: { type: 'string', description: 'Optional support filename or subpath within the skill directory, e.g. "SKILL.md", "GUIDE.md", or "references/<file>". Omit to load all available support files.' },
      },
      required: ['skill_id'],
      additionalProperties: false,
    },
...
function buildTools(): AgentTool[] {
  return [
    ...
    buildMcpCallTool(),
    // 联网工具组：只读，risk 按 read 处理（无需确认）；未配置 key 时由工具返回引导文案
    adaptLegacyTool({
      name: 'web_search',
      title: '联网搜索',
      description: 'Search the public web via the configured provider (Tavily / Bocha) and return normalized results with title, URL, snippet and optional published date. Use for recent events, company facts and any information not in local notes.',
      category: 'web',
      risk: 'read',
    ...
    adaptLegacyTool({
      name: 'web_fetch',
      title: '抓取网页正文',
      description: 'Fetch an http/https URL and extract the main text content (truncated to 20000 characters). Use to read the full content of a specific page, e.g. a result from web_search.',
      category: 'web',
      risk: 'read',
      legacy: webFetchTool,
    }),
