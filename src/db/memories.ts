...
export async function upsertMemory(
  ...
  if (similarMemory && maxSimilarity >= SIMILARITY_THRESHOLD) {
    ...
    // 同 id 原地更新语义：新内容直接替换旧记忆所在行，
    // replaced_id 自引用没有意义，置为 null（列与接口字段保留以兼容旧数据）
    await db.execute(
      `update memories set content = $1, embedding = $2, category = $3,
       replaced_id = null, updated_at = $4 where id = $5`,
      [memory.content, embeddingStr, category, now, newId]
    )
  } else {
    // 插入新记忆
    newId = generateUUID()
...
export async function getMemoryStats(): Promise<{
  ...
  // SQL 聚合统计，避免全表加载（含 embedding 大 JSON）
  const db = await getDb()
  const result = await db.select<{
    ...
    `select count(*) as total,
       coalesce(sum(case when category = 'preference' then 1 else 0 end), 0) as preferences,
       coalesce(sum(case when category = 'memory' then 1 else 0 end), 0) as memories,
       coalesce(sum(access_count), 0) as totalAccessCount
     from memories`
  )

  return result[0] ?? { total: 0, preferences: 0, memories: 0, totalAccessCount: 0 }
}
