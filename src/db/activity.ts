...
// 回填版本号：结构变化需要重新回填时递增
const ACTIVITY_BACKFILL_VERSION = 1
...
export async function initActivityDb() {
  ...
  const meta = await db.select<{ value: string }[]>(
    "select value from activity_meta where key = 'backfill_version'"
  )
  const backfilledVersion = meta[0] ? Number(meta[0].value) : 0
  if (backfilledVersion < ACTIVITY_BACKFILL_VERSION) {
    await backfillActivityEvents()
    await db.execute(
      "insert into activity_meta (key, value) values ('backfill_version', $1) on conflict(key) do update set value = excluded.value",
      [String(ACTIVITY_BACKFILL_VERSION)]
    )
  }
}
