...
export async function getTags() {
  ...
  // 一条聚合查询获取各 tag 的 marks 数量（deleted = 0），避免 N+1
  const counts = await db.select<{ tagId: number; total: number }[]>(
    "select tagId, count(*) as total from marks where deleted = $1 group by tagId",
    [0]
  )
  const countMap = new Map(counts.map(item => [item.tagId, item.total]))
